...
Firstly, configure MELPA ([[https://melpa.org/#/getting-started][upstream instructions]]). Then, refresh the
local package list and install ~cern-ldap~.
...
installed directly from [[https://melpa.org/#/cern-ldap][MELPA]]. The following snippet assumes that
MELPA has already been configured and initialised ([[https://melpa.org/#/getting-started][instructions]]).
